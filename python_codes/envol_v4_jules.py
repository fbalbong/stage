...
def target_callback_1(ts, data, logconf):
    target_data['target.x'].append(data['ctrltarget.x'])
    target_data['target.y'].append(data['ctrltarget.y'])
    target_data['target.z'].append(data['ctrltarget.z'])
    target_data['target.vx'].append(data['ctrltarget.vx'])
    target_data['target.vy'].append(data['ctrltarget.vy'])
    target_data['target.vz'].append(data['ctrltarget.vz'])

def target_callback_2(ts, data, logconf):
    target_data['target.ax'].append(data['ctrltarget.ax'])
    target_data['target.ay'].append(data['ctrltarget.ay'])
    target_data['target.az'].append(data['ctrltarget.az'])
    target_data['target.roll'].append(data['ctrltarget.roll'])
    target_data['target.pitch'].append(data['ctrltarget.pitch'])
    target_data['target.yaw'].append(data['ctrltarget.yaw'])
...
if __name__ == '__main__':
    ...
    with SyncCrazyflie(uri, cf=Crazyflie(rw_cache='./cache')) as scf:
        ...
        # --- Log ctrltarget parte 1 ---
        lc_target_1 = LogConfig('Target1', period_in_ms=20)
        for v in ['ctrltarget.x', 'ctrltarget.y', 'ctrltarget.z',
                'ctrltarget.vx', 'ctrltarget.vy', 'ctrltarget.vz']:
            lc_target_1.add_variable(v, 'float')
        scf.cf.log.add_config(lc_target_1)
        lc_target_1.data_received_cb.add_callback(target_callback_1)
        lc_target_1.start()

        # --- Log ctrltarget parte 2 ---
        lc_target_2 = LogConfig('Target2', period_in_ms=20)
        for v in ['ctrltarget.ax', 'ctrltarget.ay', 'ctrltarget.az',
                'ctrltarget.roll', 'ctrltarget.pitch', 'ctrltarget.yaw']:
            lc_target_2.add_variable(v, 'float')
        scf.cf.log.add_config(lc_target_2)
        lc_target_2.data_received_cb.add_callback(target_callback_2)
        lc_target_2.start()
...
trim_data(target_data, min_length)
...
df = pd.DataFrame({
    ...
    'T': kalman_data['T'],
    'Y': kalman_data['Y'],'''
    'target.x': target_data['target.x'],
    'target.y': target_data['target.y'],
    'target.z': target_data['target.z'],
    'target.vx': target_data['target.vx'],
    'target.vy': target_data['target.vy'],
    'target.vz': target_data['target.vz'],
    'target.ax': target_data['target.ax'],
    'target.ay': target_data['target.ay'],
    'target.az': target_data['target.az'],
    'target.roll': target_data['target.roll'],
    'target.pitch': target_data['target.pitch'],
    'target.yaw': target_data['target.yaw'],
